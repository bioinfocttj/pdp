...
Si l'utilisateur en fait la demande, un stack contenant toutes les particules sélectionnées (une par slice) \textcolor{blue}{mal dit! ;-) il crée un stack par slice?} est créé.
\section{Les classes}
...
\item La classe \textcolor{blue}{AlgoFactory} permet à l'utilisateur de choisir l'algorithme qu'il souhaite utiliser\textcolor{blue}{plutot "sert de pivot au programme en fonction du choix d'algorithme de l'utiisateur (adaptation de l'interface et algorithme de piquage)}
...
\item La classe Cropper est un peu à part.\textcolor{blue}{bizarement dit, je mettrais plutot "la classe cropper est une fonction subsidiaire du piquage permettant de..."} En effet, elle permet, si l'utilisateur en fait la demande, de créer un stack dans lequel chaque image contient une particule piquée.
...
Vient ensuite la classe PickPanel qui permet de gérer\textcolor{blue}{d'adapter l'interface en fonction de l'algorithme} le panneau d'affichage qui change pour chaque algorithme. En découlent PanelDilateDiff, PanelDoG et PanelImCorr pour \textcolor{blue}{pour leurs algorithmes respectifs (dilate diff, DOG et image cor...}les algorithmes DilateDifference, DifferenceOfGaussian et ImageCorrelation respectivement.
